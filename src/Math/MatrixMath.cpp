...
// Data operators
float& Data::operator[](int i)       { return data[i]; }
float& Data::operator[](int i) const { return data[i]; }

Data& Data::operator+=(const Data &rhs){ int i; for(i = 0; i < n; ++i) data[i]+=rhs[i]; return *this; }
Data& Data::operator-=(const Data &rhs){ int i; for(i = 0; i < n; ++i) data[i]-=rhs[i]; return *this; }
Data& Data::operator*=(const float rhs){ int i; for(i = 0; i < n; ++i) data[i]*=rhs   ; return *this; }

// Mat2 operators
inline int Mat2::ind(int r, int c) { return 2*r + c; }
float& Mat2::operator()(int r, int c)       { return data[Mat2::ind(r, c)]; }
float& Mat2::operator()(int r, int c) const { return data[Mat2::ind(r, c)]; }
...
inline void swap(Data &x, int i, int j){
    _swap_temp = x[i]; x[i] = x[j]; x[j] = _swap_temp;
}
